...
const CreateAdminUI = (ip_server) => {
	// Disable background polling of GetLastCars
	...
	var PRINTING = false;
	var abrevPlanta;
	var initAdmin = false; //Variable que determina si el Admin se ha inicializado ya
	...
	async function OpenAdmin(event){
		...
		await new Promise(r => setTimeout(r, 100));
		if(!initAdmin) {SendRequestAdm('{"info":"InfoPlanta"}');}
	}
	...
	function Inicilizar(){
		...
		for (let i=0; i<modelos.length; i++){
			// Filtrar modelos especiales/de sistema que no deben mostrarse
			const modelosExcluidos = ["COMO", "JUNK", "NOTFO", "PLCER", "GHOST", "NOINS"];
			if(modelosExcluidos.includes(modelos[i].modelo)) continue;
		...
		/* BÚSQUEDA DE PAÍSES (EN PLANTAS NUEVAS SE HA SUSTITUIDO POR FILTRO AUXILIAR)
		...
		*/
		...
		// Lanzar polling solo si está habilitado
		if(!DISABLE_POLLING){ GetLastCars(); }
	}
	...
	function DibujaUltimasInspecciones(new_car){
		...
		if (AutoPrint) SendRequestAdm('{"info":"Imprimir","idInsp":"'+ 0xFFFFFFFF +'","mode":"'+mode+'","UImode":"'+UImode+'"}');
	}
	...
	function MuestraResultados() {
		...
		if(coches_encontrados.length > 0 ){
			...
			num_res.innerHTML = coches_encontrados.length + MSGAdm[3];
		}else{
	...
	function Do_imprimir(coche){
		document.getElementById("impresion").innerHTML = FillTemplate(coche, AdmTemplates["Impresion"]);
	...
	function ChangeModeSizeDefects(elm){
		...
		if(document.getElementById("dscreens").checked)  mode = FLAG_SCREENBLOB;
		else if(document.getElementById("drawdiscarded").checked) mode = FLAG_BLOBRAWDEFECTS | FLAG_SHOWDISCARDEDRAWDEFECTS;
		else if(document.getElementById("drawndiscarded").checked) mode = FLAG_BLOBRAWDEFECTS;
		else if(document.getElementById("dactualfilter").checked) mode = FLAG_BLOBRAWDEFECTS | FLAG_FILTERRAWDEFECTS;
		ActualizeAdmin();
	}
	...
	/*Funcion para mostrar un defecto*/
	function ShowDefect(id_defect){
		return 0;
		if(document.getElementById("Zoom_Inspecciones").style.visibility == 'visible'){
			let defect = coche_zoom.uiDefects[id_defect-1];
			document.getElementById("DefectInfo").style.visibility = 'visible';
			document.getElementById("DefectInfo").style.left = event.clientX + 20;
			document.getElementById("DefectInfo").style.top = event.clientY - 30;
			document.getElementById("DefectInfo").innerHTML = "Type: " + defect.type + " Zone: " + defect.zone + " X: " + defect.x + " Y: " + defect.y + " Weight: " + defect.weight + " Size: " + defect.size;
		}
	}
	function CloseDefect(id_defect){
		return 0;
		document.getElementById("DefectInfo").style.visibility = 'hidden';
	}
