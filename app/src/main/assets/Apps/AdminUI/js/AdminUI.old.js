...
const CreateAdminUI = (ip_server) => {
	...
	function Inicilizar(){
		initView3d();
		let botonBarcode = document.getElementById("barcodegun");
		botonBarcode.style.display = "none";
	...
	//-------IMPRIMIR---------//
	function imprimir(id_inspeccion){
		AutoPrint = true;
		SendRequestAdm('{"info":"Imprimir","idInsp":"'+id_inspeccion+'","mode":"'+mode+'","UImode":"'+UImode+'"}');
    ...
    let tmplimpresion = document.getElementById("tmplimpresion");
    //console.log(tmplimpresion);
    
    let images = tmplimpresion.querySelectorAll('image');
    ...
    setTimeout(function() {
         if (AutoPrint === true && AutoJPG === false) {

            window.print();

        } else if (AutoPrint === false && AutoJPG === true) {

			//window.print();

		} else if (AutoPrint === true && AutoJPG === true) {

			window.print();

		} else if (AutoPrint === false && AutoJPG === false) {
	...
	const downloadedCars = new Set();

	function descargarImpresion(dfv) {
		const impresionDiv = document.getElementById("impresion");
		const UImode = 3; 
		if (!impresionDiv) {
			console.error("El div de impresión no existe.");
			return;
		}
		SendRequestAdm('{"info":"Imprimir","idInsp":"'+ 0xFFFFFFFF +'","mode":"'+mode+'","UImode":"'+UImode+'"}');
		console.log('{"info":"Imprimir","idInsp":"'+ 0xFFFFFFFF +'","mode":"'+mode+'","UImode":"'+UImode+'"}');
				
		const dfvElement = document.getElementById("DFV");
		let dfvValue = "impresion";

		if (dfvElement) {
			const parts = dfvElement.innerHTML.split("<br>");
			if (parts.length > 1) {
				dfvValue = parts[1].trim();
			}
		}

		if (downloadedCars.has(dfvValue)) {
			console.log(`El coche con DFV ${dfvValue} ya ha sido descargado.`);
			return;
		}

		const fechaElement = document.getElementById("Fecha");
		let fechaValue = "fecha";
		let yyyy = "2025", mm = "01", dd = "01"; 

		if (fechaElement) {
			const parts = fechaElement.innerHTML.split("<br>");
			if (parts.length > 1) {
				const fechaCruda = parts[1].trim().split(" ")[0]; 
		...
		const modeloElement = impresionDiv.querySelector("#UltModelo");
		let modeloValue = "HJB"; 
		if (modeloElement) {
			const text = modeloElement.textContent;
			const match = text.match(/\b(DJB|HJB)\b/);
			if (match && match[1]) {
				modeloValue = match[1];
			} else {
				console.warn("Modelo no encontrado, usando valor por defecto.");
			}
		}
		const rutaSalida = `D:/Fotos/${yyyy}/${mm}/${dd}/${modeloValue}/${dfvValue}.jpg`;

		const contenido = impresionDiv.innerHTML;

		const nombreArchivoHtml = `${dd}_${mm}_${yyyy}_${modeloValue}_VF1${dfvValue}.html`;
		const rutaEntrada = `C:/Descargas/${nombreArchivoHtml}`;
		const blob = new Blob([contenido], { type: "text/html" });
		const url = URL.createObjectURL(blob);

		const a = document.createElement("a");
		a.href = url;
		a.download = nombreArchivoHtml;
		...
		downloadedCars.add(dfvValue);

		//SendRequestAdm(json);
		console.log("Petición enviada:", json);
	}
	...
	function ActAutoJPG() {
		let AutoJPGBoton = document.getElementById("AutoJPGBoton");
		if(AutoJPG){
			AutoJPG = false;
			AutoJPGBoton.src = "images/AutoJPGOFF.png";
			AutoJPGBoton.onmouseover = () => { AutoJPGBoton.src = "images/AutoJPGOFFH.png"; }
			AutoJPGBoton.onmouseout = () => { AutoJPGBoton.src = "images/AutoJPGOFF.png"; }
		}else{
			AutoJPG = true;
			AutoJPGBoton.src = "images/AutoJPGON.png"
			AutoJPGBoton.onmouseover = () => { AutoJPGBoton.src = "images/AutoJPGONH.png"; }
			AutoJPGBoton.onmouseout = () => { AutoJPGBoton.src = "images/AutoJPGON.png"; }
		}
	}
