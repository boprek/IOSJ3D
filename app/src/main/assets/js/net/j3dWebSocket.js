...
function send(req) {
    ...
    console.info("[WS] Enviando peticion: " + JSON.stringify(json) );
    return 0;
}
...
function connect(url) {
    ...
    socket.onopen = () => {
        ...
        // Solo reabrir la UI si el usuario ya estaba logueado *y*
        // ya había seleccionado una UI previamente
        if (window.isLoggedIn === true && window.hasSelectedUI === true) {
            UIController.showUI();
        }

        // Notificar a todos los módulos que el WS está listo (Buscador, etc.)
        openCallbacks.forEach((cb) => {
            try {
                cb();
            } catch (e) {
                console.error("[WS] Error en callback onOpen:", e);
            }
        });
    };
